...
class Human:
    ...
    def act(self):
        ...
        # TODO: Нам будет существенно проще, если мы вынесем проверку "живой?" в отдельный метод.
        #  И будет вызывать его в цикле симуляции. act у нас будет перегружаться классами-наследниками, и мы тут
        #  реально замучаемся.
        #  .
        #  СОздайте отдельный метод is_alive, который ответчает true|false, в зависимости от того жив или нет.
...
class Husband(Human):
    ...
    # TODO: перегружать не обязательно. Не найдя метод у класса Husband, интерпретатор пойдет искать его у
    #  класса-родителя - Human. Нет, если конечно дополним __str__ чем-то своим - то ладно. А просто вызывать
    #  родительский __str__ не стоит, за нас по умолчанию это сделает Интерпретатор.
    def __str__(self):
...
# TODO: Наша задача не просто сделать классы Муж и Жена, а сделать эти классы так, чтобы в случае чего от них можно было
...
# TODO: сейчас у нас как раз начинает проблема: нужно дублировать "return True\False" внутри act`ов наследников.
...
for day in range(365):
    print()
    # TODO: здесь добавить проверку "живой?", и прерывать симуляцию, если умер.
